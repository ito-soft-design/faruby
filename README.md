...
省略可能引数 (`def f(a, b = 1)`)、可変長引数、キーワード引数、レシーバを
書く呼び出し (`obj.foo`) は未対応です。
...
次の 19 個です。**これ以外を呼ぶと実行時エラーで停止します。** レシーバの
...
| `<<` | 1 | 文字列・配列 | 文字列は中身を継ぎ足し、配列は末尾に足す。レシーバを返す |
